...
def test_healthcheck(test_client):
    ...
    logger.info(
        "Healthcheck endpoint test passed with message: %s", response.data.decode()
    )
...
def test_add_student(test_client):
    ...
    response = test_client.post(
        "/api/v1/students", json={"name": "Rajesh", "age": 21, "gender": "male"}
    )
...
def test_update_student(test_client):
    response = test_client.post(
        "/api/v1/students", json={"name": "Jack Doe", "age": 30, "gender": "male"}
    )
    response_json = response.get_json()
...
def test_delete_student(test_client):
    response = test_client.post(
        "/api/v1/students", json={"name": "Jill Doe", "age": 27, "gender": "male"}
    )
    response_json = response.get_json()
    student_id = response_json["id"]
